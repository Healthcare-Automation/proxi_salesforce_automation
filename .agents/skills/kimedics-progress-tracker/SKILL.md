...
python3 ~/.Codex/tools/notion_ticket.py list --engagement kimedics

# 2. Ship something → file it, every field filled
python3 ~/.Codex/tools/notion_ticket.py create \
...
python3 ~/.Codex/tools/notion_ticket.py update --match "<words from the title>" \
