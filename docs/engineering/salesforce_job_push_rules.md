...
| `Job_Volume__c`        | — (no static default; set only when `avg_patients_per_day` is non-empty and not the literal placeholder **Not Provided**) |
...
| `avg_patients_per_day`                      | `Job_Volume__c`                                           | Parsed from description (`Avg patients per day:` / section); omitted in SF when empty or when value is the literal **Not Provided** (case-insensitive); no placeholder is sent for blank volume                                                                                                                                                       |
...
| (derived)                                   | `Name`                                                    | Header title: `{StateAbbr} ({City}) General Dentistry - {brand} - {Open|Closed}` where **brand** comes from Kimedics `posting_org` (e.g. Heartland Dental, Midwest Dental, Aspen Dental Management Inc.); **city** falls back from `practice_value` / `location_line` when `city` is blank; **no empty `()`** when city cannot be resolved (80 char cap). See `job_name_brand_display_for_row` / `build_salesforce_job_name` in `sf_job_payload.py`.                                                                                          |
...
| (derived from history)                      | `Job_Open_Date__c`                                        | Most recent date when job status changed to **Open** or **Active, accepting new providers**. Queries historical `job_content` records for status changes.                                                                                                                                                                                           |
...
- **Opening:** by default, `**PROXI_JOB_DESCRIPTION_USE_AI`** is treated as on (unset → try OpenAI intro via `job_description_ai.generate_ai_intro_html`; requires `OPENAI_API_KEY`). The model is asked for one `<p>` paragraph; multiple `<p>` blocks are merged into one. Set to `false` to use the fixed template only. On API/key failures the template falls back without failing the push.
