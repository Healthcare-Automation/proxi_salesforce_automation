...
| Concept | Default Account Id | Target API field |
|--------|-------------------|------------------|
| **Account** — Aspen Dental Management Inc. | `0015f00000HH63kAAD` | `Job_Account__c` |
| **Worksite Location 1** — default worksite account | `001UP00000HKOXRYA5` | `Job_Worksite_Location_1__c` |

**Worksite display label** (for humans, not a separate SF field in this mapping): `Aspen Dental - {City}, {State}` — see `utils.sf_push_defaults.format_worksite_display_label` and `job_sf_enrichment.enrich_cleaned_row_salesforce_fields`. The **worksite Account Id** on each job comes from Supabase `sf_worksite_account_id` (filled after Kimedics scrape via cache → `job_content` history → 1:1 SF practice match; see `utils.sf_job_supabase_resolve`).
